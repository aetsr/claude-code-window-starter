...
5. **Medium — conversion friction.** Missing clone/auth steps, duplicated builds, a rollback command without required `--yes`, and pytest instructions without the source path or pytest in dev requirements. UI and bot replies are now English-only.
